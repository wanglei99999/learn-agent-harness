...
这里是很明显的，几乎只在hook里做了注入上下文和压缩消息的操作，其余的实现都是以tool的方式接入的。

将 s01-s11 所有机制合并的终章实现。
s12（任务感知的 worktree 隔离）单独讲授。
非教学 session —— 这是"合并在一起"的参考实现。

    +------------------------------------------------------------------+
    |                        完整 AGENT                                 |
    |                                                                   |
    |  系统提示（s05 技能、任务优先 + 可选 todo 提醒）                    |
    |                                                                   |
    |  每次 LLM 调用前：                                                  |
    |  +--------------------+  +------------------+  +--------------+  |
    |  | 轻压缩 (s06)        |  | 排空后台通知      |  | 读信箱       |  |
    ...
    |  +--------+----------+----------+---------+-----------+          |
    |                                                                   |
    |  Subagent (s04)：  spawn → 工作 → 返回摘要                         |
    |  Teammate (s09)：  spawn → 工作 → 空闲 → 自动认领 (s11)            |
    |  Shutdown (s10)：  request_id 握手                                 |
    |  Plan gate (s10)： 提交 → 批准/拒绝                                 |
    +------------------------------------------------------------------+
...
class TeammateManager:
    ...
    def _loop(self, name: str, role: str, prompt: str):
        ...
        # teammate 独立的系统提示；权限比 lead 小，无 plan_approval/broadcast/shutdown_request 等管理工具
        sys_prompt = (f"You are '{name}', role: {role}, team: {team_name}, at {WORKDIR}. "
                      f"Use idle when done with current work. You may auto-claim tasks.")
        messages = [{"role": "user", "content": prompt}]
        # teammate 只暴露执行类工具，send_message 只能点对点，无法广播或发起协议握手
        tools = [
        ...
        while True:
            # -- 工作阶段（最多 50 轮，兜底防止无限循环）--
            for _ in range(50):
                # hook：每轮开头 drain 收件箱；shutdown_request 立即退出，其余消息注入 messages
            ...
            for _ in range(IDLE_TIMEOUT // max(POLL_INTERVAL, 1)):
                ...
                if inbox:
                    ...
                    resume = True
                    break
                # s11：扫描任务板，认领第一个无主且无阻塞的 pending 任务
                unclaimed = []
                ...
                if unclaimed:
                    ...
                    # messages 过短说明经历过压缩，重注入身份防止 LLM 忘记自己是谁
                    if len(messages) <= 3:
        ...
        for block in response.content:
            if block.type == "tool_use":
                ...
                if block.name == "TodoWrite":
                    used_todo = True
        # s03：Nag reminder（仅在有未完成 todo 时生效）
